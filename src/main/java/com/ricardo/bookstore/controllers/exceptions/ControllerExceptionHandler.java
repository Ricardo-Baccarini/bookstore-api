...
public class ControllerExceptionHandler {
	...
	@ExceptionHandler(DataIntegrityViolationException.class) // Manipulador de exceção
	public ResponseEntity<StandardError> dataIntegrityViolationException(DataIntegrityViolationException e, ServletRequest request) {
		StandardError standarErro = new StandardError(System.currentTimeMillis(), HttpStatus.BAD_REQUEST.value(),
				e.getMessage());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(standarErro);
	}
